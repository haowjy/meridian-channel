...
### 3. Use `<br/>` for line breaks — never `\n`

`\n` is **not supported** by Mermaid. Use `<br/>` (self-closing) for line breaks in all diagram types. Remember that labels containing `<br/>` must be quoted (Rule 1).

```mermaid
%% ✅ GOOD — <br/> inside quoted label
A["Turn 1<br/>user message"]

%% ✅ ALSO GOOD — markdown string with literal newline (flowcharts only, v10+)
B["`Turn 1
user message`"]

%% ❌ BAD — \n does nothing, rendered literally
C["Turn 1\nuser message"]
```

**Flowchart tip:** Markdown strings (`` ["`...`"] ``) support literal newlines plus `**bold**` and `*italic*` formatting, but only work in flowchart node labels, edge labels, and subgraph labels.
