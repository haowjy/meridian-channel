...
def generate_space_summary(
    *,
    ...
    markdown = _render_summary_markdown(record, runs=run_store.list_runs(space_dir))
...
def collect_space_markdown_artifacts(
    *,
    ...
    for run in run_store.list_runs(space_dir):
        report_path = SpacePaths.from_space_dir(space_dir).runs_dir / run.id / "report.md"
