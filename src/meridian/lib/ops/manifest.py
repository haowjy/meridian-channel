...
_OPERATIONS: tuple[OperationSpec[Any, Any], ...] = (
    ...
    _spec(
        name="hooks.check",
        description="Verify runtime requirements for all built-in hooks.",
        handler=hooks_check,
    ...
    _spec(
        name="hooks.list",
        description="List registered hooks with event, source, and status.",
        handler=hooks_list,
    ...
    _spec(
        name="hooks.resolve",
        description="Resolve enabled hooks for one event in execution order.",
    ...
    _spec(
        name="hooks.run",
        description="Manually execute one hook by name; bypasses interval throttling.",
        handler=hooks_run,
