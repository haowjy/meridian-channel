...
_OPERATIONS: tuple[OperationSpec[Any, Any], ...] = (
    ...
    _spec(
        name="spawn.files",
        description="List files touched by a spawn, one per line. Pipe to git add or xargs.",
        handler=spawn_files,
        sync_handler=spawn_files_sync,
        input_type=SpawnFilesInput,
        output_type=SpawnFilesOutput,
        cli_group="spawn",
        cli_name="files",
    ...
    _spec(
        name="work.done",
        description='Mark a work item as done (equivalent to `work update --status "done"`).',
        handler=work_done,
    ...
    _spec(
        name="work.start",
        description="Create a work item and set it active for the current session when available.",
        handler=work_start,
