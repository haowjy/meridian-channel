...
def test_run_app_uds_forwards_socket_path_and_project_uuid(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    project_root = tmp_path / "project"
    ...
    socket_path = tmp_path / "socket" / "meridian.sock"

    app_cmd.run_app(
        uds=str(socket_path),
        allow_unsafe_no_permissions=False,
    )
    ...
    assert fake_uvicorn.calls == [
        (
            app_object,
            {"uds": str(socket_path), "log_level": "info"},
        )
    ]
    assert not (runtime_root / "app.port").exists()


def test_run_app_non_default_host_uses_tcp_with_default_port(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    project_root = tmp_path / "project"
    runtime_root = tmp_path / "runtime"
    project_uuid = "project-uuid-789"
    app_object = object()
    captured, fake_uvicorn = _setup_common_monkeypatches(
        monkeypatch=monkeypatch,
        project_root=project_root,
    ...
    app_cmd.run_app(host="0.0.0.0", allow_unsafe_no_permissions=True)
