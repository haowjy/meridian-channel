...
def test_discover_items_uses_conventional_layout(tmp_path: Path) -> None:
...
def test_discover_items_ignores_nonconforming_files(tmp_path: Path) -> None:
    tree = tmp_path / "source"
    (tree / "agents").mkdir(parents=True)
