...
def test_run_wait_sync_emits_heartbeat_when_verbose_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        run_ops,
    ...
    monkeypatch.setattr(
        run_ops,
        "_emit_wait_heartbeat",
        lambda message, *, sink: heartbeats.append(message),
    )
    ...
    assert heartbeats[0].startswith("waiting ")
    assert "pending spawns (1): r1" in heartbeats[0]


def test_run_wait_sync_default_mode_has_no_heartbeat(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        run_ops,
