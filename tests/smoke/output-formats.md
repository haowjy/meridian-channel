...
```bash
uv run meridian --json spawn -p "format probe" --dry-run > /tmp/meridian-formats-json.json && \
python3 - <<'PY'
import json
...
```bash
uv run meridian --format json spawn -p "format probe" --dry-run > /tmp/meridian-formats-format-json.json && \
python3 - <<'PY'
import json
