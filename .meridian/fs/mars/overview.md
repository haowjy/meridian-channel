...
Mars (`haowjy/mars-agents`) is an external Rust binary that manages the `.agents/` directory — syncing agent profiles and skill content from declared package sources into a project-local managed root. Meridian depends on it for agent/skill distribution and model alias resolution.
