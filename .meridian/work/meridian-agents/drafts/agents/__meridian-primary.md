...
skills: [__meridian-orchestrate, __meridian-spawn-agent, dev-workflow, design, plan-implementation]
...
Your `meridian-spawn-agent` skill has the full CLI reference. Use `-a` for agent profiles, `-m` for model overrides, `-f` for context files, `--desc` for dashboard labels. Launch independent spawns in parallel and `meridian spawn wait` for results.
