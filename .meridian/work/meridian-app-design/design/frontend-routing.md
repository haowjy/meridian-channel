...
**wouter** (~1.5kb gzipped) provides `useRoute`, `useLocation`, and a `<Route>` component — everything needed for SPA routing without the overhead of react-router's data loading, nested layouts, or action patterns.

Dependency: `wouter` (add via `pnpm add wouter`).

## URL Routes

| Path | Component | Purpose |
|------|-----------|---------|
| `/` | `<Dashboard />` | Work-item-centric dashboard with active work and quick sessions |
| `/s/:sessionId` | `<SessionView />` | Thread view for one session |
| `/w/:workId` | `<WorkItemView />` | Detail view for a work item (all sessions, metadata) |
| `/sessions` | `<SessionList />` | All sessions view (filterable) |
...
```
App.tsx                           ← Shell: activity bar + layout + router
├── ActivityBar.tsx               ← Left icon bar (48px)
├── Sidebar.tsx                   ← Work-item-grouped session list (264px)
│   ├── SidebarHeader.tsx
│   ├── WorkSection.tsx
│   │   ├── WorkItemGroup.tsx
│   │   └── SessionRow.tsx
│   └── QuickSection.tsx
├── FileExplorer.tsx              ← Multi-root file tree (collapsible panel)
│   ├── ExplorerHeader.tsx
│   ├── RootNode.tsx
│   └── TreeNode.tsx
├── MainPane.tsx                  ← Route container (flex-1)
│   ├── Dashboard.tsx             ← Route: /
│   │   ├── ActiveWork.tsx        ← Work item cards
│   │   ├── QuickSessions.tsx     ← Unattached session grid
│   │   └── DashboardComposer.tsx ← New session composer
│   ├── SessionView.tsx           ← Route: /s/:sessionId
│   │   ├── SessionHeader.tsx
│   │   ├── SpawnTree.tsx         ← Child spawn visualization
│   │   ├── ThreadView.tsx
│   │   └── Composer.tsx
│   ├── WorkItemView.tsx          ← Route: /w/:workId
│   │   ├── WorkItemHeader.tsx
│   │   └── SessionList.tsx
│   ├── AllSessionsView.tsx       ← Route: /sessions
