...
This work item depends on the **launch-core-refactor** work item completing first. A04's workspace projection plugs into the launch-domain core's `apply_workspace_projection()` pipeline stage. Without the single-composition-seam invariant that launch-core-refactor delivers, A04 would need to wire workspace projection into every driving adapter independently.
