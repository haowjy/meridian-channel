...
The restructure has two generations of decisions. The **v2 generation (D1–D15)** built the orchestrator topology and autonomous redesign loop. The **v3 generation (D16–D26)** reframes the design package around spec-driven development (SDD), which reshapes several v2 decisions in place. Within v3, D16–D23 defined the core SDD reshape and D24–D26 close the convergence-review findings (dev-principles scope, EARS mechanical parsing across all five patterns, preserved-phase re-verification on revised leaves).
...
- **Contract decisions** — D12 caps the planner cycle within a single impl-orch run with a two-counter scheme (K_fail=3 for failed plans + K_probe=2 for probe-requests) plus a `structural-blocking` short-circuit that bypasses both counters, escalating as `planning-blocked` — distinct from D7's outer redesign cap. D13 extracts the Terrain section into a standalone shared artifact contract (revised by D19/D20 — the single Terrain section splits into three named outputs). D14 defines the preservation-hint as a data contract dev-orch produces between cycles (the "verified scenarios" field is replaced by "spec leaves satisfied (stable/revised flagged)" per D22). D15 picks the terminated-spawn model for plan review pause/resume, matching meridian's crash-only design philosophy.

**v3 SDD reframe (D16–D26):**
...
- **Convergence-review closures (D24–D26)** — D24 scopes the `dev-principles` gate to design-orch only; impl-orch and @planner carry the principles as context, not as convergence gates. D25 extends D17/D21 with a per-pattern EARS parsing guide so all five EARS patterns map mechanically to trigger/precondition/response (not just the three that carry an explicit WHEN or WHILE clause). D26 adds the preserved-but-requires-reverification mechanism for redesign cycles: preserved phases whose claimed leaves were revised in place get a tester-only re-verification pass against the existing commits, closing the silent-drift hole Fowler's spec-anchored discipline is meant to prevent.

Themes:
- **Topology** — D1, D2, D3, D6
- **Runtime/planning boundary** — D3, D4, D12, D15
- **Redesign loop** — D5, D6, D7, D8, D9, D14, D26
- **Parallelism & structure** — D10, D11, D13
- **SDD shape (v3)** — D16, D17, D18, D19, D20, D21, D22, D23, D24, D25
...
**Reasoning.** Bail-out is expensive. Triggering on severity makes it too frequent; triggering on evidence type makes it proportional to the actual problem. A smoke test revealing a protocol contradicts the design is execution-time falsification; a unit test failing because a fixture is stale is a bug. A planner that exhausts K_fail (three failed plans) or K_probe (two probe-requests), or that emits a `structural-blocking` short-circuit, is planning-time falsification; a planner that needed one re-spawn for a missing spec leaf or a single probe-request answered from runtime context is normal planning iteration. Impl-orch must justify in the redesign brief why the evidence is falsification, and a brief that cannot make that case is rejected. The planning-time arm uses the same justification burden — the brief's "Parallelism-blocking structural issues" section is the planning-time analog of the falsification case section.
...
**Reasoning.** Each committed phase represents work that has verified at least one EARS statement against the spec leaves it claimed. Default-preserve makes the cost of redesign proportional to the scope of the actual change. Impl-orch's second run can skip preserved phases and start from the first invalidated phase forward.
...
1. The architecture tree's root overview (`design/architecture/overview.md`) includes the current structural posture, the target structural posture, and the structural delta. The structural delta's refactor entries land in `design/refactors.md` as named work, and the "does the target state fix or preserve the existing structural problems?" verdict lives in `design/feasibility.md` §"Fix-or-preserve verdict" alongside the probe evidence backing it. Together these three surfaces replace what v2 kept in a single Terrain section of `design/overview.md`, and downstream consumers (reviewers, impl-orch, planner) read them as three named files rather than as sections of one page.
...
3. Convergence criteria expand from functional-only to functional + structural + `dev-principles`-compliant. A design that converges with reviewers but leaves the system as coupled as it found it — or that violates any project-wide structural principle without a documented override — is treated as not-yet-converged.
...
## D12: Planning cycle is bounded by two independent counters — `K_fail`=3 failed plans and `K_probe`=2 probe-requests

**Decision.** Impl-orch maintains two independent counters over the planner spawns in a single impl-orch cycle. `K_fail` counts planner spawns whose output was a plan that impl-orch rejected as incomplete or incoherent (missing required sections, missing or duplicated spec-leaf claims, unaccounted `refactors.md` entries, contradictions with pre-planning notes, hand-wavy parallelism justifications); it caps at **3**. `K_probe` counts planner spawns that terminated with a probe-request report instead of a plan; it caps at **2**. A probe-request does **not** advance `K_fail`, because the planner is asking for inputs it did not have rather than failing to converge on inputs it did have; and a plan-rejection does not advance `K_probe`. Exits: a convergent plan advances neither counter and accepts the plan; a plan with `Parallelism Posture: sequential` + `Cause: structural coupling preserved by design` advances neither counter and routes `structural-blocking` to dev-orch immediately (the short-circuit); exhausting `K_fail` fires `planning-blocked`; exhausting `K_probe` fires `planning-blocked` with `Cause: probe-request loop`.

**Precedence with structural-blocking.** If any spawn's plan carries `Parallelism Posture: sequential` with `Cause: structural coupling preserved by design` (including the foundational-prep variant), impl-orch emits `structural-blocking` only. The short-circuit bypasses both counters — re-spawning the planner against the same design cannot resolve the coupling, so the fail-counter bookkeeping is beside the point. This rule wins even if the same spawn's plan also has completeness failures that would normally advance `K_fail`. See impl-orchestrator.md §"Planning cycle cap" for the impl-orch side and planner.md §"Planning-cycle cap" for the planner side.

**Alternatives rejected.**
- *A single unified counter (v0 K=3).* The v0 scheme lumped probe-requests and plan-rejections into one cap. Reviewer feedback flagged the false-positive mode: a planner working a 20+ leaf spec tree could burn all three slots on probe-requests (different runtime gaps each round), never producing a bad plan, and trigger `planning-blocked` purely from pre-planning incompleteness. Separating the counters means a probe-request loop still has its own cap (preventing infinite probing) without forcing rejection-like treatment on a spawn that is asking legitimate questions.
- *No cap at all.* Reviewer feedback on the v1 design package flagged unbounded planner re-spawn as a pathological-loop risk.
- *`K_fail`=1 or 2.* Too tight. A normal planning iteration (fixing a missing spec-leaf claim or a hand-wavy round justification) is a single re-spawn, and a second iteration to address a follow-up gap is normal. `K_fail`=3 leaves room for two real corrections plus an unsuccessful third before escalating.
- *`K_probe`=3 or higher.* Too loose. A planner that cannot decide after two rounds of added runtime context is almost certainly asking questions that the pre-planning notes should have front-loaded, or the gap is design-side and needs dev-orch routing. `K_probe`=2 gives room for one legitimate iteration plus a follow-up.
- *`K_fail`=5 or higher.* Too loose. Beyond three failed convergence attempts, the gap is almost always in the inputs (design or pre-planning notes), not in the planner's craft. Dev-orch should be the one to decide whether to revise inputs or accept partial planning.
...
**Reasoning.** The planning cycle cap is the planning-time analog of D7's redesign cycle cap. They count separately — D7 caps redesign cycles across the work item, D12 caps planner spawns within a single impl-orch cycle. Exhausting either counter fires the `planning-blocked` signal which routes to dev-orch via the redesign loop and may or may not advance D7's counter depending on how dev-orch routes it. Without D12, the planner could loop indefinitely on a design or pre-planning gap that requires escalation to fix.

Separating `K_fail` from `K_probe` protects against the pre-planning-gap false positive the single-counter scheme had: a work item where pre-planning notes are thin but the design itself is fine shouldn't trigger a design revision cycle (which `planning-blocked` can cause) just because the planner asked for three different runtime probes in a row. With the split, probe-requests consume their own budget, and the `K_fail` counter stays reserved for plans that are genuinely non-convergent.
...
**Decision.** The required shapes of `design/refactors.md` and `design/feasibility.md` live in `design/terrain-contract.md` as a standalone artifact contract. The contract specifies the required per-entry fields for each refactor (ID, target, affected callers, coupling removed, must-land-before, architecture anchor, behavior-preservation flag, evidence), the disambiguation rule for refactors versus foundational prep, the canonical section layout of `feasibility.md` (probe results, parallel-cluster hypothesis, foundational prep, fix-or-preserve verdict with reasoning, known unknowns), and the evidence requirements each entry must meet. The producer (design-orchestrator.md) and consumers (impl-orchestrator.md, planner.md, structural reviewer brief) all reference the contract rather than re-stating it.

**Alternatives rejected.**
- *Keep the content inline in design-orchestrator.md (the v1 draft).* Reviewer feedback flagged that this tangles producer responsibility with consumer expectations. A planner reading planner.md would have to also read design-orchestrator.md to know what to expect from refactors.md and feasibility.md, and a structural reviewer would have to read both to know what to check. Updates to the contract would land in one body and silently drift from the others.
...
- *Keep the single-file Terrain-section shape from v2.* The v2 Terrain section mixed four concerns (current+target posture, structural delta, probe evidence, fix-or-preserve verdict) into one page and tagged refactor candidates with `structural-prep-candidate: yes|no`. v3 promotes each concern into a named artifact with a distinct consumer — structural posture in the architecture tree (D18), refactor agenda in `refactors.md` (D19), feasibility evidence and foundational prep in `feasibility.md` (D20). The `structural-prep-candidate` tag is retired because every entry in `refactors.md` is a refactor by construction, and every entry under `feasibility.md §"Foundational prep"` is foundational prep by construction; the tag was a shared-file hack that disappears once the files are separate.
...
**Decision.** After every successful design-orch revision cycle, dev-orch writes `plan/preservation-hint.md` per the format in `design/preservation-hint.md`. The hint carries the preserved-phase list (with commit SHAs and spec leaves satisfied, flagged per-leaf as `stable` or `revised: <reason>`), the partially-invalidated list (with what is invalid and what is salvaged), the fully-invalidated list, the `replan-from-phase` anchor, the new or revised spec leaves from the redesign cycle, and the constraints-that-still-hold from the brief. The hint is overwritten each redesign cycle, not appended — cycle history lives in the brief and decisions.md. The next planning impl-orch consumes the hint as the first thing it reads in pre-planning, scopes runtime probing to the invalidated portion, and passes the hint to the planner via `-f`.
...
**Reasoning.** Default-preserve is the central claim of the autonomous redesign loop. Without a concrete data contract, the next impl-orch cycle has no structured way to know which phases to skip, which scope to replan from, and which spec leaves are still claimed by preserved phases (versus revised in place and needing re-verification). The hint makes the claim auditable and gives every consumer a single source of truth for the preservation state.
...
**Decision.** Every leaf in `design/spec/` is written as an EARS statement — one of the five EARS patterns (Ubiquitous, State-driven, Event-driven, Optional-feature, Complex) — with an assigned ID. EARS is not a suggestion; it is the mandated shape, enforced by the **spec reviewer** during design convergence (see [design-orchestrator.md](design/design-orchestrator.md) §"Reviewer fan-out" — the spec reviewer's focus areas include EARS concreteness, testability, coverage, and ambiguity; the alignment reviewer checks cross-artifact consistency, not EARS shape). An EARS leaf looks like `WHEN <trigger> WHILE <state> THE <system> SHALL <response>` or one of the other patterns; the ID format is `S<subsystem>.<section>.<letter><number>` (e.g. `S03.1.e1` for section 3.1's first event-driven leaf).
...
- *Let reviewers flag ambiguity ad-hoc instead of mandating a shape.* Makes the convergence gate non-deterministic — two reviewers would disagree on "is this clear enough" and the spec reviewer's judgment would be the only check. An enforced shape is a deterministic gate.
...
**This revises D13.** D13 extracted terrain analysis into a standalone shared artifact contract. D19 keeps that intent (shared contract, producer/consumer separation) but splits the single Terrain section into three outputs: (1) `refactors.md` holds the refactor agenda — this decision; (2) `feasibility.md` (D20) holds gap-finding, probe evidence, the parallel-cluster hypothesis, and the foundational-prep scaffolding catalog in §"Foundational prep"; and (3) the architecture tree's target-state sections (D18) hold the structural posture — the target-state interfaces and types that foundational prep must satisfy, but not the scaffolding entries themselves. The architecture tree answers "what should the contracts look like"; `feasibility.md §'Foundational prep'` answers "which scaffolds need to be built to realize those contracts". D13's "one shared contract doc" remains — `terrain-contract.md` — but it now describes the shape of the three outputs rather than a single flat section.
...
**This revises D13** alongside D19. D13's single Terrain section is now three outputs — `refactors.md` (rearrangement agenda), `feasibility.md` (gap-finding, probes, parallel-cluster hypothesis, foundational-prep scaffolding catalog, fix-or-preserve verdict), and the architecture tree's target-state sections (structural posture and the interface contracts foundational prep satisfies). The shared artifact contract still exists (`terrain-contract.md`) but now governs the shape of the three outputs rather than a single flat section.
