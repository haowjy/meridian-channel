...
1. **Completeness** — required sections present, every refactors.md entry mapped to a phase or a skip decision, every EARS statement in `design/spec/` (at `S<subsystem>.<section>.<letter><number>` ID granularity, not leaf-file granularity — a single spec leaf may contain multiple EARS statements that end up in different phases) claimed by exactly one phase in `plan/leaf-ownership.md`, parallelism justification per round.
...
Planner re-spawns are bounded by **two independent counters** with distinct semantics. See [planner.md](planner.md) §"Planning-cycle cap" for the planner-side contract; this section covers the impl-orch side.

- **`K_fail` — failed-plan counter, capped at 3.** Counts planner spawns whose output was a plan that failed completeness or consistency — missing required sections, spec leaves unclaimed or double-claimed, `refactors.md` entries unmapped, contradictions with pre-planning notes, or hand-wavy parallelism justifications that do not cite real constraints. Impl-orch evaluates every returned plan and advances `K_fail` when it rejects the plan with concrete feedback that the planner would have to revise.
- **`K_probe` — probe-request counter, capped at 2.** Counts planner spawns that terminated with a probe-request report rather than a plan. A probe-request does **not** advance `K_fail` — the planner is asking for inputs it did not have, not failing to converge on inputs it did have. Impl-orch runs the requested probes, appends the results to `plan/pre-planning-notes.md`, and re-spawns. The separate cap exists because the inputs actually change between probe-request rounds, so D12's "same inputs, same failure" logic does not apply.

Exits from the planning loop:

1. **Convergent plan.** Advances neither counter. Impl-orch accepts the plan and moves on to the pre-execution structural gate. Normal exit.
2. **Structural-blocking short-circuit.** A plan that ships with `Parallelism Posture: sequential` and `Cause: structural coupling preserved by design` (including the foundational-prep variant) advances **neither** counter. Structural-blocking is a design-side signal, not a planner failure — re-spawning against the same design cannot resolve it. Impl-orch routes the structural-blocking escalation to dev-orch immediately, bypassing `K_fail`/`K_probe` entirely. If both signals apply — a plan is incomplete *and* flags structural-blocking — structural-blocking wins and the re-spawn is skipped regardless of remaining cap slots.
3. **`K_fail` exhausted.** After three failed plans in a row, impl-orch escalates to dev-orch with a `planning-blocked` signal. The escalation is a terminal report citing the last-attempt artifact, the gap reasoning impl-orch provided on each re-spawn, and the conclusion that planner convergence is not achievable with the current design + pre-planning notes. Dev-orch decides whether to revise the design (back through design-orch) or to accept the partial plan with explicit known gaps.
4. **`K_probe` exhausted.** After two probe-requests in a row without a convergent plan, impl-orch escalates to dev-orch with a `planning-blocked` signal and `Cause: probe-request loop`. The runtime data the planner keeps asking for is either unknowable or design-relevant, and the loop has to unwind at the dev-orch level.

`K_fail` and `K_probe` are independent. A work item can burn one probe-request, then one failed plan, then a convergent plan; that is two spawns total but neither counter exhausted. A work item can burn three failed plans with no probes; `K_fail` exhausts and the escalation fires regardless of unused `K_probe` slots. Impl-orch maintains both counters explicitly in its execution-loop bookkeeping.

These caps are **distinct from the redesign cycle cap (D7)**. They count separately — exhausting either planner counter fires the `planning-blocked` signal; exhausting the redesign cycle cap is the absolute outer bound on the autonomous loop. See `decisions.md` D12 for the cap rationale and `decisions.md` D7 for the outer bound.

**Cluster-hypothesis front-loading.** For work items with more than ten spec leaves, the pre-planning notes **must** include a "Spec-leaf coverage hypothesis" section clustering the leaves into parallel-eligible groups before the planner is spawned. This front-loads the cluster analysis the planner would otherwise have to do via probe-request rounds, and protects both counters from being exhausted on pre-planning gaps the notes should have caught. For work items with ten or fewer spec leaves, the hypothesis is optional — the planner can do the cluster analysis in-context without burning a cap slot.
...
### Preserved phases with revised leaves — re-verification, not re-implementation

The preservation hint uses `preserved` to mean "code committed, do not respawn coder" — but some preserved phases own spec leaves whose EARS statement was revised in the current redesign cycle (the ID stays stable, the trigger/precondition/response text changed). The hint marks those leaves with `revised: <reason>` in the preservation section (see [preservation-hint.md](preservation-hint.md) §"Status field representation"). If impl-orch skips preserved phases entirely, revised leaves ride the old verification and the system silently drifts from the new spec — this is the exact failure mode Fowler's spec-anchored discipline exists to prevent.

The mechanism: preserved phases whose hint lists any `revised:` spec leaves get a **tester-only re-verification pass**. No coder is respawned, the existing commits stand, but impl-orch spawns the testers named in the phase blueprint with a scoped instruction to re-verify the revised leaves against the current code. Three outcomes:

1. **All revised leaves still verify.** The code already happened to satisfy the new EARS statement (common when revisions tighten language without changing behavior). Re-verification passes, phase stays `preserved`, execution continues.
2. **One or more revised leaves falsify.** The old code does not satisfy the new EARS statement. Impl-orch promotes the phase from `preserved` to `partially-invalidated` in `plan/status.md`, respawns the phase coder with a scoped instruction naming the now-failing revised leaves, and re-runs the normal fix loop. The preserved commits are **not** reverted — the coder lands an additional delta on top of them.
3. **Re-verification cannot be executed** (the testers cannot run, environment is broken, etc.). Impl-orch treats this as a planning-time blocker: promote the phase to `replanned`, emit a redesign brief naming the blockage, and let dev-orch route. This should be rare.

The re-verification pass runs **before** replanned and new phases begin, because those phases may depend on contracts that revised leaves describe. Running them against un-re-verified preserved code would stack uncertainty on top of uncertainty.

Impl-orch emits one tester spawn per preserved phase with at least one revised leaf; phases with zero revised leaves skip the re-verification pass entirely and run as pure `preserved` (no coder, no tester). This is the "preserved but requires re-verification" category the preservation hint contract references.

### Decision log, status, and context handoffs

The decision log, status tracking, and context-handoff patterns carry over unchanged. On a redesign cycle, the execution-loop impl-orch reads the preservation hint, runs the re-verification pass described above for preserved phases with revised leaves, skips preserved phases whose leaves were not revised, treats `partially-invalidated` phases as needing revision (re-spawns the coder with the partial-invalidation scope), and runs the `replanned` and `new` phases as fresh work. Leaf ownership survives the redesign cycle via the hint's preservation section — leaves claimed by preserved phases remain claimed; leaves affected by the revision get reclaimed by the replanned phases or re-verified in place via the pass above.
...
2. **Planner cannot converge within the planning caps.** Exhausting either `K_fail=3` (failed plans) or `K_probe=2` (probe-requests) fires the `planning-blocked` signal and emits a redesign brief naming the gap reasoning that prevented convergence. See "Planning cycle cap" above for the cap mechanics.
...
After all phases pass phase-level verification, the final review loop runs across the full change set as today — reviewer fan-out across diverse models, one refactor reviewer, design-alignment reviewer, iterate until convergent. Design alignment at this stage checks the full implementation against the spec tree: every **EARS statement** in `design/spec/` (at `S<subsystem>.<section>.<letter><number>` granularity, matching `plan/leaf-ownership.md`) has been verified by at least one phase, and no committed code introduces behavior the spec does not describe.

**`dev-principles` as a reviewer lens at the final review.** The same principles gate design-orch applies during design convergence ([design-orchestrator.md](design-orchestrator.md) §"`dev-principles` as convergence gate") is re-applied at this stage against the committed implementation — but impl-orch does **not** run a pass/fail gate itself. Instead, impl-orch spawns one reviewer in the final fan-out with a brief that names `dev-principles` as the review lens: did the committed change set leave refactor discipline unfinished, skip edge cases the spec enumerates, introduce abstractions at fewer than three call sites, trip structural health signals (module size, import growth, greppability drop), or ship integration boundaries without protocol probing? The reviewer returns findings the same way any other reviewer does, and impl-orch feeds them back into the standard reviewer-response loop until convergence. This keeps the principles active at execution time without turning every impl-orch phase boundary into a gate pass, and it reuses the existing reviewer mechanism instead of introducing a parallel gate (see [decisions.md](../decisions.md) D24).
...
- The two-counter planning cycle cap (K_fail=3 for failed plans + K_probe=2 for probe-requests, with a `structural-blocking` short-circuit that bypasses both) and the `planning-blocked` escalation signal.
