...
1. **Bail-out trigger.** Which planning-time signal fired:
   ...
   - `planning-blocked` — planner cycle cap was exhausted without convergence. Name which counter exhausted: K_fail=3 (three failed plans) or K_probe=2 (two probe-requests). Cite each spawn's failure mode and the gap reasoning impl-orch provided on each re-spawn. (The `structural-blocking` trigger above is the short-circuit variant and fires independently of the two counters.)
