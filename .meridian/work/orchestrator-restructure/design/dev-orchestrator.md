...
3. Planning impl-orch terminates with one of three terminal report shapes:
   ...
   - **planning-blocked** — planner re-spawn cycle cap exhausted (either K_fail=3 failed plans or K_probe=2 probe-requests, or a `structural-blocking` short-circuit bypassing both counters); report cites the planning-time redesign brief.
...
4. **Spec-leaf coverage is complete and exclusive at EARS-statement granularity.** `plan/leaf-ownership.md` claims every **EARS statement** in `design/spec/` (at `S<subsystem>.<section>.<letter><number>` ID granularity, not leaf-file granularity) by exactly one phase. A single leaf file may contain multiple EARS statements that end up owned by different phases — that is legal, as long as every statement has exactly one owner. Unclaimed statements mean the plan does not cover the design; duplicated statements mean the plan has a coverage collision. Both are bugs.
...
- **planning-blocked** — planning impl-orch's planner cycle cap was exhausted without convergence. Either K_fail=3 failed plans, or K_probe=2 probe-requests, or the planner emitted a `structural-blocking` short-circuit that bypasses both counters. Brief cites the planner's last-attempt artifact and the gap reasoning impl-orch provided on each re-spawn.
...
The redesign cycle counter (this section, K=2) is **distinct from the planning cycle cap** (the two-counter scheme in [impl-orchestrator.md](impl-orchestrator.md) §"Planning cycle cap" — K_fail=3 failed plans + K_probe=2 probe-requests, with a `structural-blocking` short-circuit that bypasses both). The planning cap counts planner re-spawns within a single impl-orch cycle. The redesign cap counts design-orch re-spawns across the whole work item. They count separately — exhausting either planning counter, or receiving a `structural-blocking` short-circuit, fires `planning-blocked` (which advances the redesign counter only if dev-orch then routes it to a design-orch cycle). See `decisions.md` D12 for the split-counter rationale.
